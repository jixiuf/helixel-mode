...
| Module | Role |
|--------+------|
| ~helixel-debug.el~ | Error capture: debug toggle, error ring with backtraces, ~helixel--with-debug-log~ (cl-lib only) |
| ~helixel-core.el~ | Pure data layer: structs, registries, tx helpers, replay context (→ debug) |
| ~helixel-motion.el~ | Motion recording/repeat: last-motion struct, motion repeater registry (→ core) |
| ~helixel-register.el~ | Named registers: backends, current-register, delete-register rotation (→ core) |
| ~helixel-ring.el~ | Event storage: ring + jump-log + tracking-open + live-event + grouped-ring queries |
...
| Struct | Role | Mutable? |
|--------+------+----------|
| ~helixel-sel~ | Selection descriptor (typed struct per kind; behavior via ~cl-defgeneric~ methods) | Immutable (copy on update) |
...
observers — chain (~helixel-repeat.el~) hooks into it to append the
...
function that delegates to the kind's ~helixel-sel-advance-fn~ method.
All-buffer and all-dir scans pull ~:all-buffer-fn~ / ~:all-dir-fn~
from the corresponding protocol methods.
...
Each selection kind defines a ~helixel-sel-advance-fn~ method (and
optionally ~helixel-sel-all-buffer-fn~ / ~helixel-sel-all-dir-fn~).
...
The authoritative require graph is generated from the source:

: make depgraph

See [[file:DEPGRAPH.org]] (regenerated by ~make depgraph~).  It is a
DAG — no circular requires.
...
helixel-repeat.el:helixel--repeat-advance(edit, effective)
  ├── Delegates to helixel-sel-advance-fn method
  ...
  │           └── calls :runner — named function symbol (from the action,
  │                 falling back to the op registry)
  │
...
- The runner/preposition stored in the event struct are named function
  symbols (pure data): ring entries stay printable, ~equal~-comparable,
  and serializable.  Replay falls back to the op registry when an
  action's runner slot is nil.
