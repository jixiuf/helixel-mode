...
This is the opposite of Vim's operator-pending model (~d3w~ = "delete 3 words"
where the operator comes first).  In Helixel you type ~w w w d~ — select three
words, then delete.  If you're coming from Vim, this is the one paradigmatic
difference to internalize.
...
#+begin_src elisp
  ...
  (helixel-mode)
#+end_src
You may try helixel by running:
#+begin_src sh
make run
# or
...
Some settings most users change:

#+begin_src elisp
  ;; Enable PCRE-style regexp in search (/ and ?) via pcre2el
  ;; You need install the optional pcre2el
...
| ~.~                     | Repeat last edit. Prefixes: ~3.~ = 3 times; ~0.~ = all remaining; ~C-u .~ = entire buffer; ~C-u - 3 .~ = 3 times reversed. Works for all selection kinds (line, search, movement w/e/b, textobj, find-char). |
| ~M-.~                   | Repeat last selection without editing (same prefix behavior as ~.~), this is a preview of ~.~                                                                                                                |
| ~g.~                    | Repeat by picking from history                                                                                                                                                                               |
| ~i~ ~a~ ~I~ ~A~ ~o~ ~O~ | Enter insert mode                                                                                                                                                                                            |
| ~v~                     | Enter visual mode                                                                                                                                                                                            |
| ~d~ ~c~ ~y~ ~r~ ~R~     | Edit: delete, change, copy, replace, replace-char                                                                                                                                                            |
| ~S~                     | Swap regions (see [[#swap][Swap]])                                                                                                                                                                                      |
| ~p~ ~P~                 | Paste after / before; pasted text tracked for ~;~ / ~C-;~ re-select                                                                                                                                          |
| ~<~ ~>~                 | Indent left / right:  you can repeat next lines by ~.~ :x4> ....                                                                                                                                             |
| ~u~ ~U~                 | Undo / redo                                                                                                                                                                                                  |
| ~f~ ~t~ ~F~ ~T~         | Find char                                                                                                                                                                                                    |
| ~/~ ~?~                 | Search by regexp(toggle regexp by M-r)                                                                                                                                                                       |
| ~n~ ~N~                 | Repeat (support repeat of search and find char) / reverse direction repeat (~C-u n~ pick from history)                                                                                                   |
| ~,~                     | Repeat last motion (f, t, /, ?, %, [, ], {, })                                                                                                                                                               |
| ~;~                     | Action cycle (mark-thing) — see [[#action-cycle--and-jump-cycle-c-][Action cycle / Jump cycle]]                                                                                                                                                    |
| ~C-;~                   | Jump cycle (original position) — see [[#action-cycle--and-jump-cycle-c-][Action cycle / Jump cycle]]                                                                                                                                               |
...
| ~mi~ ~ma~               | Text objects (word, symbol, sentence, paragraph, pairs, quotes, tags)                                                                                                                                        |
| ~ms~ ~mt~               | Surround add / add tag                                                                                                                                                                                       |
| ~md~ ~mr~               | Surround delete / replace                                                                                                                                                                                    |
| ~[~ / ~]~ / ~{~ / ~}~   | Pair & structure movement prefixes (see below)                                                                                                                                                               |
| ~s~                     | Multi-cursor prefix                                                                                                                                                                                          |
...
~;~ and ~C-;~ navigate the event history ring (=helixel--action-ring=).
Both commands visit the same events but differ in what they mark:

- ~;~ (action cycle): The first press after a movement *selects the full
  span* (mark-thing) — the region from the far edge of the thing to point.
  Subsequent presses cycle to older events, pushing mark to the event start.
- ~C-;~ (jump cycle): *Every press pushes mark to the original pre-motion
  cursor position* (start-point).  It never selects the full span.
  Subsequent presses cycle to older events.

Both commands share the same grouping logic: consecutive movements of the
same kind (e.g. ~ww~) form a group, and the cycle shows the group-start.
They maintain independent cycle positions, so pressing ~;~ a few times
does not affect where ~C-;~ will start.
...
stored in the event's =:mark-region=.  The first ~;~ after a paste or
replace jumps back to the edit spot and marks the full span of the
pasted/replaced text, just like after a word or pair movement.
Subsequent ~;~ presses cycle through older session starts as usual.
Note: the default =helixel-action-cycle-categories= includes only the
paste/replace/yank-pop edit subcats; kill, change and others are
excluded by default (add = (edit . kill)= etc. to include them).
...
| find-char         | =find-char=           | ~f~ ~F~ ~t~ ~T~ then ~n~/~N~/~,~                |
| movement          | =pair= =match=        | ~%~ ~[~p ~]~p then ~,~                          |
| edit-paste        | =edit-paste-after=    | ~p~ ~P~ — ~;~ re-selects pasted text            |
| edit-replace      | =edit-replace=        | ~r~ — ~;~ re-selects replaced text              |
...
*Example 1* — Buffer ="hello world"=, cursor at =hel|lo= (position 4):
- Press ~ww~ — two word-forward movements, point lands at buffer end.
- Press ~;~ — selects the full span: ="hello world"= (1..12).
- Press ~C-;~ instead — selects ="lo world"= (4..12), from the original
  cursor position to the end of the last word.
...
Both ~;~ and ~C-;~ accept a ~C-u~ prefix to go forward (newer) instead
of backward.
...
(helixel-define-key 'textobj-inner "o"    #'helixel-mark-inner-org-block 'org-mode)
(helixel-define-key 'textobj-outer "o"    #'helixel-mark-a-org-block 'org-mode)

;; Markdown ``` fences (counter-based: name-group nil)
(helixel-define-regex-textobj md-fence  "^```[^\n]*$" "^```[ \t]*$" nil 'block)
(helixel-define-key 'textobj-inner "o"    #'helixel-mark-inner-md-fence)
(helixel-define-key 'textobj-outer "o"    #'helixel-mark-a-md-fence)
...
Requires [[https://github.com/meain/evil-textobj-tree-sitter][evil-textobj-tree-sitter]] (it does NOT depends on evil-mode)
...
- Press ~;~ after a movement to select the full thing (mark-thing).
  Subsequent ~;~ presses cycle to older events.  Press ~C-;~ to
  push mark to the event start position instead (without selecting
  the full span).  Both commands navigate the same event history.

  Available categories and subcats:
...
**** PCRE Support (pcre2el)

When =helixel-search-pcre= is non-nil and the
[[https://github.com/joddie/pcre2el][pcre2el]] package is
available, PCRE-style regexp patterns (=\d=, =\w=, =\s=, etc.) are
translated to Emacs-compatible equivalents before searching.  This
works for both interactive isearch (~/ / ~?~) and ~n~/~N~ repeat.

If pcre2el is not installed, patterns are passed through unchanged
and interpreted as standard Emacs regexp — no error is raised.

#+begin_example
  /\d{3}-\d{4}<RET>   search for 3 digits, hyphen, 4 digits
  n                    next match forward
#+end_example

Supported PCRE shorthand classes:
...
| =\b=   | =\b= / =\B=     | Word boundaries (both forms)  |

Enable it globally:

#+begin_src elisp
  ;; Require pcre2el first, then enable PCRE in helixel search
  (require 'pcre2el)
  (setq helixel-search-pcre t)
#+end_src
...
*** Dot-Repeat (~.~) and Repeat-Selection (~M-.~)

After an edit, ~.~ repeats the edit at the next target.
~M-.~ repeats the selection only (preview, no edit) — same prefixes.

| Prefix            | Behavior                                                |
|-------------------+---------------------------------------------------------|
| ~.~ ~M-.~         | Repeat once in stored direction                         |
| ~3.~ ~3 M-.~      | Repeat 3 times in stored direction                      |
| ~0.~ ~0 M-.~      | Repeat all remaining targets in stored direction        |
| ~C-u .~ ~C-u M-.~ | Repeat all targets, entire buffer (point-min → forward) |
| ~-.~ ~- M-.~      | *Permanently* reverse direction, then 1 repeat          |
| ~-3.~ ~-3 M-.~    | *Permanently* reverse direction, then 3 repeats         |
...
~@~ starts recording, ~ESC~ finishes and creates a repeatable
compound transaction.  ~.~ advances to the next target and replays
the entire sequence.  Both helixel commands (movement, edit, state)
and vanilla Emacs commands are captured.

| Key   | Action                                                                   |
|-------+--------------------------------------------------------------------------|
| ~@~   | Start chain recording (snapshots selection context, starts tx recording) |
| ~ESC~ | End chain, create repeatable compound transaction                        |
| ~.~   | Advance to next target and replay the recorded sequence                  |
| ~M-.~ | Preview the next target without editing                                  |
| ~C-g~ | Cancel chain (discard recording)                                         |

*How it works:*

1. Select a target (line with ~x~, search match with ~/~) — this
   establishes the /advance context/.
2. Press ~@~ — starts recording, snapshots the selection.
3. Perform any editing operations — all commands (helixel and vanilla
   Emacs) are captured as replayable entries in chronological order.
...
| Helixel movements (w)  | ~action-commit-hook~                     | Selection recreate (positioning)       |
| Insert-mode typing     | ~insert-record~ (after-change-functions) | Text segments verbatim                 |
| Vanilla commands (C-a) | ~post-command-hook~                      | ~execute-kbd-macro~ with original keys |

Insert-mode commands (~RET~, ~DEL~, etc.) are captured exclusively by
the insert-recording mechanism and NOT duplicated as vanilla entries.
This prevents double-replay (e.g. two newlines instead of one).
...
#+begin_src elisp
  ;; Exclude additional commands from vanilla capture
...
~,~ repeats the most recent motion
self-contained from its recording — it does not depend on =helixel--active-search=
or the current region.  Supported motion types:
...
#+begin_example
  ...
  ,   → outward to ) after (b)
  ,   → outward to ) after (a)
  ;; ; now marks the outermost (a ...) pair
#+end_example

This works through mixed bracket types (~( ] }~) and across block delimiters
(~#+begin_src / #+end_src~):
...
#+begin_example
  ...
  (let ((x 1))
    (message x))
  #+end_src
  ;; % inside (message x) → jump to matching )
  ;; , → outward to enclosing (let ...)
  ;; , → outward to #+begin_src
#+end_example
***** ~,~ with ~;~

Every motion repeated by ~,~ records its own entry in the action ring,
complete with span bounds (=:mark-region=).  Pressing ~;~ after ~,~
selects the full span that was just traversed — exactly like pressing
~;~ after the original motion.

*Layer-by-layer pair selection* (~%~ + ~,~ + ~;~):

After ~%~ jumps to the matching delimiter, ~,~ expands outward one
nesting level.  Each ~,~ press records the enclosing pair's bounds, so
you can inspect any level:
...
#+begin_example
  hello world
  fx    → jump to x in "world"
  ,   → find next x
  ;     → select the span from origin to latest x match
#+end_example

In every case, the ~;~ command reads the most recent event's span
bounds and activates a region covering the full traversed distance.
Subsequent ~;~ presses cycle through older events as usual.

**** find-char: ~,~ vs ~n~

Both ~n~ and ~,~ repeat the last find-char, but they differ in one key way:
~,~ reads from a self-contained snapshot taken at record time, so it survives
across an intervening search (~/~).  ~n~ consults =helixel--active-search= and
will switch to repeating the search instead.
...
~,~ uses the stored delimiter bounds to skip past the current pair entirely
before re-invoking the original command.  This means consecutive ~,~ presses
walk through consecutive pairs rather than re-executing on the same one:
...
#+begin_src elisp
  ;; Categories and subcats that , can repeat.
  ;; Format: (CATEGORY . SUBCAT) or just CATEGORY.
  ;; A plain symbol matches all subcats under that category;
  ;; a cons matches only the specified subcat.
...
| Key          | Action                                                               |
|--------------+----------------------------------------------------------------------|
| ~ss~         | Toggle (spawn from last selection / clear)                           |
| ~s,~         | Clear all fake cursors (Helix ~,~ — keep primary only)               |
| ~sa~ / ~sA~  | Add a fake cursor at point, move real to next / previous line        |
| ~sx~         | Per-line cursors: line-mode → full-line REGION on each line          |
|              | (Helix ~Alt-s~); char-mode → POINT at current column on each line    |
| ~sn~ / ~sp~  | Add cursor at next / previous occurrence of region text              |
| ~sN~ / ~s P~ | Skip next / previous match without adding a cursor                   |
| ~su~ / ~sU~  | Unmark next / previous fake cursor                                   |
| ~s.~         | Apply =helixel--last-event= at every fake cursor                     |
| ~sk~ / ~sK~  | Keep / remove cursors whose region matches a regex (Helix ~K~ ~M-K~) |
| ~sS~         | Split each selection on regex (Helix ~S~)                            |
| ~s-~         | Merge all cursors into one big region (Helix ~M--~)                  |
| ~s&~         | Column-align cursors by padding with spaces (Helix ~&~)              |
| ~s_~         | Trim leading / trailing whitespace from each region (Helix ~_~)      |
...
Chain on N cursors (~@~):
#+begin_example
  /TODO<RET> ss @ miw c BUG <ESC> A ! <ESC> ESC
...
Each fake cursor owns its own =helixel--action-ring=, =--live-event=,
...
- ~@ ... ESC~ while multi-cursor is active: the resulting chain
