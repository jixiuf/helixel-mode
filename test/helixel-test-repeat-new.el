...
modify `helixel--pending-sel'."
  (helixel-test-with-buffer "hello world foo bar"
    ...
    (let ((stored-sel (copy-sequence (helixel-event-sel helixel--last-tx))))
      ;; Replay
      (goto-char 1)
      (helixel-repeat-edit)
      ...
      (should (equal (helixel-event-sel helixel--last-tx) stored-sel)))))
...
Cursor-offset is 0 (first insertion at region-end)."
  (let ((helixel-repeat-change-method 'text))
    (helixel-test-with-buffer "hello world hello"
      ...
      (should (string= (plist-get (helixel-event-payload helixel--last-tx) :text)
                       "foo"))
      (let ((sel (helixel-event-sel helixel--last-tx)))
