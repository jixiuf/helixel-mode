...
#+begin_src elisp
(:category search|find-char|next-error :pattern PAT :dir forward|backward [:type TYPE :char CHAR])
#+end_src
...
- *next-error*: ~n~/~N~ call ~next-error~ / ~previous-error~ when
  ~next-error-last-buffer~ is a compilation buffer.  Compilation
  window stability is enforced via ~display-buffer-overriding-action~.
  In-buffer navigation (mc spawn / mark-like-this) saves-restores
  compilation buffer state to avoid corrupting the global
  ~next-error~ cursor.
...
helixel-repeat.el:helixel--repeat-advance(edit, effective)
  ...
  │     ├── next-error: call `next-error' then recreate
