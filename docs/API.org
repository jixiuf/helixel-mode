...
| ~:skip-reverse-exchange~ | boolean | skip `exchange-point-and-mark' on N reverse |
...
- ~CATEGORY~ — e.g. ~'movement~, ~'search~, ~'next-error~
- ~SUBCAT~ — e.g. ~'pair~, ~'match~, ~'word~, or ~nil~
...
(setq helixel-motion-repeat-categories
  '((movement . pair) (movement . match) search find-char next-error))
#+end_src
