...
(ert-deftest helixel-test-edit-make-stores-display ()
  "`helixel-action-create' stores :display in DISPLAY-FIELD slot, not in :payload."
...
(ert-deftest helixel-test-execute-edit-uses-stored-runner ()
  "`helixel--execute-action' calls the :runner stored in TX."
  (with-temp-buffer
    (insert "hello")
    (goto-char 1)
...
(ert-deftest helixel-test-execute-edit-fallback-registry ()
  "`helixel--execute-action' falls back to registry when :runner missing."
  ...
  (with-temp-buffer
    ...
    (let ((tx (helixel-action-create 'kill nil)))
      ...
      (should (progn (helixel--execute-action tx) t)))))
...
(ert-deftest helixel-test-repeat-edit-preserves-on-error ()
  ...
  (helixel-test-with-buffer "hello"
    (setq helixel--last-action
          (helixel-action-create 'kill (helixel-sel-create 'unknown-kind-no-method nil #'ignore "?")))
    (let ((before helixel--last-action))
      (helixel-repeat-edit)
      (should (equal helixel--last-action before)))))
...
(ert-deftest helixel-test-repeat-selection-count ()
  ...
  (helixel-test-with-buffer "line one\nline two\nline three\n"
    (goto-char 1)
    (setq helixel--last-action
          (helixel-action-create 'kill (helixel-sel-create 'line '(:count 1) #'helixel--recreate-line "L")))
