;;; helixel-mc-spawn.el --- Spawn fake cursors from selections -*- lexical-binding: t; -*-
...
;; Spawning real fake cursors from helixel selections.
;;
;; Entry points:
;;   helixel-mc-spawn-from-sel  — generic, dispatches on kind
;;   helixel-mc-spawn-from-line — column cursors on each selected line
;;   helixel-mc-spawn-from-rect — column cursors over a rect region
;;   helixel-mc-add-cursor-here — toggle a fake cursor at point
...
;;   helixel-mc-edit-lines     — one cursor per line of region
;;   helixel-mc-toggle         — high-level: from selection / from last edit
;;
;; Generic strategy: when a kind provides `:mc-spawn-fn' in the kind
;; registry, use it.  Otherwise walk the kind's `:advance' function
;; from `point-min' collecting every advance landing point, then
;; turn each into a fake-cursor — except the one nearest the real
;; cursor, which stays as the real cursor.
...
No-op outside an mc session."
  ...
  (unless (helixel-mc-any-p)
    (user-error "No fake cursors"))
  (let* ((real-keep (string-match-p regex
    ...
    (unless real-keep
      (if (null survivors)
          (helixel-mc-clear-all)
        (let* ((promote (car survivors)))
          (helixel-mc--promote-fake-to-real promote))))
...
Inverse of `helixel-mc-keep-matching'.  No-op outside mc."
  ...
  (unless (helixel-mc-any-p)
    (user-error "No fake cursors"))
  (let* ((real-drop (string-match-p regex
                                    (helixel-mc--current-real-region-text))))
    (dolist (ov (helixel-mc-all-cursors))
      (when (string-match-p regex (helixel-mc--cursor-region-text ov))
        (helixel-mc-delete-fake-cursor ov)))
    (when real-drop
      (let ((survivors (helixel-mc-all-cursors)))
        (if (null survivors)
            (helixel-mc-clear-all)
  ...
  (dotimes (_ (or count 1))
    (let* ((sorted (helixel-mc-all-cursors :sort))
           ...
           (prev (or (cl-find-if
                      ...
                      (reverse sorted))
                     (car (last sorted)))))
      (helixel-mc--swap-real-and-fake prev))))
