* helixel-mode User Guide :TOC_4_gh:
  - [[#philosophy][Philosophy]]
  - [[#installation][Installation]]
    - [[#via-use-package-with-vc-emacs--30][Via use-package with :vc (Emacs ≥ 30)]]
  ...
  - [[#selection--movement][Selection & Movement]]
    ...
    - [[#goto-prefix-g][Goto Prefix (~g~)]]
    - [[#view--scroll][View / Scroll]]
    - [[#pair--structure-movement-------][Pair & Structure Movement: ~[~ / ~]~ / ~{~ / ~}~]]
  - [[#editing][Editing]]
    - [[#edit-commands][Edit Commands]]
    - [[#registers][Registers]]
      - [[#how-registers-work][How Registers Work]]
      - [[#register-backends][Register Backends]]
      - [[#default-register-vs-explicit-register][Default Register vs Explicit Register]]
      - [[#how-deletes-populate-registers][How Deletes Populate Registers]]
  ...
  - [[#search--find][Search & Find]]
    ...
    - [[#occur-integration][Occur Integration]]
  ...
  - [[#action-cycle---mark-cycle-c-][Action Cycle (~;~) & Mark Cycle (~C-;~)]]
    ...
    - [[#session-groups][Session Groups]]
      - [[#movement-sessions][Movement sessions]]
      - [[#search--find-sessions][Search & find sessions]]
      - [[#text-object-sessions][Text-object sessions]]
      - [[#edit-sessions-each-operation-is-its-own-session-group][Edit sessions (each operation is its own session group)]]
  ...
  - [[#multi-cursor-s][Multi-Cursor (~s~)]]
    ...
    - [[#managing-cursors][Managing Cursors]]
      - [[#s-prefix-always-available-in-normal-mode][~s~ prefix (always available in normal mode)]]
      - [[#top-level-keys-always-available-enable-mc-automatically][Top-level keys (always available, enable mc automatically)]]
      - [[#top-level-keys-when-multi-cursor-is-active][Top-level keys (when multi-cursor is active)]]
      - [[#top-level-rotation-keys-when-multi-cursor-is-active][Top-level rotation keys (when multi-cursor is active)]]
    - [[#workflows][Workflows]]
  ...
  - [[#motion-repeat-][Motion Repeat (~,~)]]
    ...
    - [[#mc-spawn-multi-cursor-repeat][mc-spawn: Multi-Cursor Repeat]]
...
| ~g｜~        | Move to column             |
| ~g v~  ~s v~ | Restore last cursor layout |
...
| ~y~ | prev class           | next class (treesit) |
| ~,~ | prev parameter       | next parameter (treesit) |
| ~l~ | prev loop            | next loop (treesit) |
| ~e~ | prev conditional     | next conditional (treesit) |
| ~o~ | prev sibling         | next sibling (treesit) |

Inner sub-keys (~{~ / ~}~) for treesit objects:

| Key | ~{~ inner start          | ~}~ inner end            |
|-----+--------------------------+--------------------------|
| ~f~ | prev function body       | next function body end  |
| ~y~ | prev class body          | next class body end     |
| ~,~ | prev parameter (tight)   | next parameter (tight)  |
| ~l~ | prev loop body           | next loop body end      |
| ~e~ | prev conditional body    | next conditional body end |
...
helixel provides built-in tree-sitter support via
~helixel-treesit.el~ — no external package required:

#+begin_src elisp
  (require 'helixel-treesit)
  ;; Setup is automatic when helixel-mode activates.
  ;; Manual call (idempotent) only needed if you install
  ;; treesit after helixel-mode is already on:
  ;; (helixel-treesit-setup)
#+end_src

This registers semantic text objects and navigation keys that
work in any treesit-enabled buffer.  Query resolution delegates
to ~evil-textobj-tree-sitter~ (soft dependency, 151+ languages).
See [[file:TREESIT.org][TREESIT.org]] for the full keybinding table.

For custom per-language bindings without ~helixel-treesit-setup~,
you can still use the low-level helper:
...
If ~evil-textobj-tree-sitter~ is not installed, ~helixel-get-tree-sitter-textobj~
returns ~nil~ and bindings are silently ignored.
...
| ~n~     | After compile/grep hit | Repeat next-error forward            |
| ~N~     | After compile/grep hit | Reverse direction then repeat        |
...
M-g n       next-error (compile/grep)
;; Or just hit RET on a match in a compile/grep buffer.
n           next match forward via next-error
N           reverse direction, previous match via previous-error
,           repeat the last next-error motion
#+end_src

When a compilation, grep, occur, or diff buffer is the
~next-error-last-buffer~, ~n~ and ~N~ navigate matches via
~next-error~ / ~previous-error~ instead of searching.  ~,~
(motion repeat) also works — it remembers the direction and
replays the cross-buffer jump.  ~s s~ (mc toggle) spawns fake
cursors at every compilation match in the current buffer.
...
[[https://github.com/joddie/pcre2el][pcre2el]] package:
...
After running =M-x occur= (or =multi-occur=,
=multi-occur-in-matching-buffers=, =occur-revert=), the regexp is
automatically stored so that ~n~, ~N~, and ~,~ can repeat-search for
it:

- ~n~ / ~N~ :: repeat-search for the occur regexp in the source
  buffer (the buffer where ~occur~ was invoked).
- ~,~ :: also works — in the source buffer, in the =*Occur*= buffer,
  and even after entering =occur-edit-mode= (~e~ in the occur
  buffer).  The search state survives major-mode changes via
  =occur-revert-arguments=.

No configuration needed — the integration wires itself automatically
when =helixel-mode= is active.
...
| ~class~ ~parameter~ ~loop~ ~conditional~ ~sibling~ ~grow-shrink~ | tree-sitter objects (custom bindings) |
...
| treesit (~maf~ ~mif~ ~may~) | one cursor per semantic object (function,     |
|                             | class, parameter, loop, conditional, comment) |
...
Treesit bulk editing:
#+begin_src
maf s s i DEBUG: <ESC>
# select function → spawn at every function → insert prefix on each
#+end_src

Treesit mark-like-this (semantic s n):
#+begin_src
maf s n s n d
# select func1 → mark func2 → mark func3 → delete all 3 functions
# (each in separate undo step equivalent)
#+end_src
...
| next-error | —                                     | ~M-g n~ → ~,~ jump next match              |
...
*next-error context*: When the active search is a `next-error' repeat
(after ~M-g n~ / ~M-g p~ or when a compilation buffer is active),
~s n~ / ~s p~ / ~s N~ / ~s P~ navigate compilation matches within the
current buffer instead of using literal text search.  ~s s~ spawns
fake cursors at every compilation match in the current buffer.
...
[[https://github.com/jixiuf/emacs-leadkey][leadkey]].
