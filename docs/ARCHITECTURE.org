...
#+begin_src
helixel-core (→ helixel-debug; includes replay context)
  ├── helixel-motion (→ core)     ; consumed by move/search/engine/mc/...
  ├── helixel-register (→ core)   ; consumed by search/editing
  ├── helixel-ring (→ core)
