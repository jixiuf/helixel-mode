...
The three settings most users change:

#+begin_src elisp
  ;; Cursor: bar in insert, box in normal (default)
  (setq helixel-mode-cursor-color "#ff8f00")

  ;; Enable PCRE-style regexp in search (/ and ?) via pcre2el
  (setq helixel-search-pcre t)

  ;; Add mode-specific keybindings
  (helixel-define-key 'normal "SPC f" #'find-file)
#+end_src

See ~M-x customize-group RET helixel RET~ for all 26 options.
...
| ~;~                    | Action cycle: first press selects the full span, subsequent presses cycle older events |
| ~C-;~                  | Jump cycle: push mark to original pre-motion cursor position, cycle events          |
| ~C-o~ ~C-i~           | Jump to older / newer position (global, cross-buffer)            |
| ~M-.~  ~'~            | Repeat last motion (f, t, %, [, ], {, })                        |
...
| ~;~                      | Action cycle (mark-thing) — see [[Action cycle / Jump cycle]]   |
| ~C-;~                    | Jump cycle (original position) — see [[Action cycle / Jump cycle]] |
...
*Example:* Buffer ="hello world"=, cursor at =hel|lo= (position 4).
...
| Key   | Action                                                                       |
|-------|------------------------------------------------------------------------------|
| ~q~   | Start chain recording (snapshots selection context, starts tx recording)     |
| ~ESC~ | End chain, create repeatable compound transaction                            |n| ~.~   | Advance to next target and replay the recorded sequence                      |
| ~,~   | Preview the next target without editing                                      |n| ~C-g~ | Cancel chain (discard recording)                                            |

*How it works:*

1. Select a target (line with ~x~, search match with ~/~) — this
   establishes the /advance context/.
2. Press ~q~ — starts recording, snapshots the selection.
...
| Search (~/~)    | Next match                                                      |n| None            | In-place (no advance, replay at cursor position)               |
...
| movement          | =pair= =match=        | ~%~ ~[~p ~]~p then ~M-.~                        |
...
~M-.~ (and ~'~ — same command) repeats the most recent motion
...
| Kind            | Subcat                         | Example                                       |
|-----------------+--------------------------------+-----------------------------------------------|
| find-char       | =next= / =till=                | ~f x~ → ~M-.~ finds next x (like ~n~)         |
| movement        | =pair=                         | ~]~ → ~M-.~ skip past next paren boundary     |
| movement        | =match=                        | ~%~ → ~M-.~ expand one nesting level outward  |
| movement        | =paragraph= / =sentence= / =function= | ~}~ → ~M-.~ skip past next boundary    |
| search          | —                              | ~/foo~ → ~M-.~ find next match (like ~n~)     |

**** Layer-by-layer outward (~%~ + ~M-.~)

Inside a nested pair, ~%~ jumps to the matching delimiter.  ~M-.~ then moves
...
#+begin_example
  (a (b (c)))
        ↑
  %     → jump to ) after (c)
  M-.   → outward to ) after (b)
  M-.   → outward to ) after (a)
...
#+begin_example
  ...
  ;; M-. → outward to enclosing (let ...)
  ;; M-. → outward to #+begin_src
#+end_example

**** find-char: ~M-.~ vs ~n~

Both ~n~ and ~M-.~ repeat the last find-char, but they differ in one key way:
~M-.~ reads from a self-contained snapshot taken at record time, so it survives
across an intervening search (~/~).  ~n~ consults =helixel--active-search= and
will switch to repeating the search instead.

#+begin_example
  f x        find x — recorded for M-.
  /pattern   active-search now points to search, not find-char
  M-.        still finds the next x (from snapshot)
  n          repeats /pattern (from active-search)
#+end_example

**** For pair motions (~]~ ~[~ ~}~ ~{~)

~M-.~ uses the stored delimiter bounds to skip past the current pair entirely
before re-invoking the original command.  This means consecutive ~M-.~ presses
...
#+begin_src elisp
  ;; Categories and subcats that M-. and ' can repeat.
